...
let find_widget id =
    ...
    try
        Simulation.all () |>
        List.find_map (fun s -> Widget.find s.root id) |>
        Option.some
    with Not_found -> None
...
|};
    ...
    let all_widgets =
        Simulation.all () |>
        List.map (fun s -> Widget.descendants s.root) |>
